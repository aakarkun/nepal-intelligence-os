/**
 * Circuit breaker: after `threshold` consecutive failures, the source is
 * suspended. Auto-reset after 1 hour (single retry).
 */
...
export function getAllCircuitBreakerStates(): CircuitBreakerState[] {
  return Array.from(breakers.values()).map((cb) => cb.getState());
}
